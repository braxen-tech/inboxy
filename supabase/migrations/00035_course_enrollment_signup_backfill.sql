-- Extend handle_new_auth_user to also backfill course_enrollments.end_user_id
-- when a buyer who purchased a course before creating an account signs up.
...
BEGIN
  ...
  IF v_role = 'end_user' THEN
    ...
    -- Backfill: link digital product purchases made with this email before signup.
    UPDATE public.digital_product_purchases
    SET end_user_id = NEW.id
    WHERE buyer_email = NEW.email AND end_user_id IS NULL;

    -- Backfill: link course enrollments made with this email before signup.
