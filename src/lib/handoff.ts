...
export function buildHandoffSystemInstructions(
  ...
  const lines = [
    ...
    `- Se o cliente pedir humano sem critério de roteamento, chame transfer_to_human SEM assignee_name (fila geral / Unassigned).`,
  ...
  if (availableAgents.length > 0) {
    lines.push(
      `- Atendentes disponíveis nesta conta Chatwoot: ${availableAgents.map((a) => `"${a.name}"`).join(", ")}.`,
    );
  } else {
    lines.push(
      `- Nenhum atendente humano foi encontrado na conta Chatwoot. Sem assignee_name, a conversa irá para a fila geral após handoff.`,
    );
  }
