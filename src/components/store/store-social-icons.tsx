...
const SVG_ICON_MAP: Record<string, React.FC<{ className?: string; style?: React.CSSProperties }>> = {
  ...
  return (
    <div className="flex items-center justify-center gap-3">
      {links.map(({ platform, url }) => {
        const SvgIcon = SVG_ICON_MAP[platform];
        const LucideIconComp = LUCIDE_ICON_MAP[platform];
        ...
        return (
          ...
          >
            {SvgIcon ? (
              <SvgIcon className="size-5" style={{ color: "var(--store-text)" }} />
            ) : LucideIconComp ? (
              <LucideIconComp className="size-5" style={{ color: "var(--store-text)" }} />
            ) : (
              <Globe className="size-5" style={{ color: "var(--store-text)" }} />
            )}
          </a>
        );
      })}
