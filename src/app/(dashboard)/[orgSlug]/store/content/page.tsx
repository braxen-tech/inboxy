...
export default async function StoreContentPage({ params }: Props) {
  ...
  const [{ data: blocks }, { data: digitalProducts }, { data: courses }, { data: banner }] = await Promise.all([
    ...
    db.from("store_banners").select("id, text, link_url, link_product_id, link_course_id, link_label, visible_from, visible_until, active").eq("organization_id", org.id).maybeSingle(),
  ]);
